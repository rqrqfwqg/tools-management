...
| 服务 | 端口 | 访问地址 | 说明 |
|------|------|---------|------|
| 后端 API | 3000 | `http://<IP>:3000` | 生产模式同时提供静态文件 |
| PC 前端 | 3100 | `http://<IP>:3100` | Vue3 + Element Plus |
| 手机前端 | 3200 | `http://<IP>:3200` | Vue3 + Vant UI，适配手机浏览器 |

局域网访问示例：`http://192.168.3.4:3200`（手机版）
...
> `npm run build` 会把 Vue 前端编译成静态文件，输出到 `vue-frontend/dist/`，后端会自动加载。移动端前端同理（`mobile-frontend/`）。
