...
export function getEnteredCodes(checkId: number): Set<string> {
  try {
    const raw = localStorage.getItem(`${KEY_PREFIX}${checkId}`)
...
/** 标记某编码已录入（幂等；localStorage 不可用时静默降级，仅影响标记不影响盘库正确性） */
export function markEntered(checkId: number, code: string): void {
  try {
    const set = getEnteredCodes(checkId)
    set.add(code)
    localStorage.setItem(`${KEY_PREFIX}${checkId}`, JSON.stringify([...set]))
  } catch {
